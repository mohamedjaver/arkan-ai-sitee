/* ═══════════════════════════════════════════════════════════════
   BDL Intelligence Archive — Receipts PDF Report
...
window.ArkanPdfReport={ run: function(docs,opts){
  ...
  try{
    ...
    function header(first){
      ...
      pdf.text('BDL INTELLIGENCE',M,44);
      ...
      pdf.text('Receipts Report - lbdal.com',W-M,60,{align:'right'});
    ...
    pdf.text('BDL Intelligence Archive', W/2, H-50, {align:'center'});
    pdf.setFont('helvetica','normal'); pdf.setFontSize(8.5); pdf.setTextColor(MUTED);
    pdf.text('AI OCR 2.0 · lbdal.com', W/2, H-38, {align:'center'});
