...
module.exports = function (app, ctx) {
  ...
  async function writeReport() {
    ...
    const brief = { date: new Date().toISOString().slice(0, 10), profits: deals ? (deals.ops ? { source: 'console_settlement', unit: 'MRU', today: Math.round(deals.ops.today), month: Math.round(deals.ops.month), all_time: Math.round(deals.ops.all), closed_ops: deals.ops.n, note: 'هذا هو الربح الفعلي المسجّل في كونسول التسوية' } : { source: 'pair_deals', unit: deals.unit, today: deals.today, month: deals.month, deals: deals.n, unpriced_deals: deals.unpriced, avg_margin_pct: deals.avgMargin }) : null, escalation: esc ? esc.levels : null, receipts_total: s.total, customers: { receipts: s.custN, sum_aoa: fmt(s.custSum), open_without_supplier: s.open - s.openSup, open_sum_aoa: fmt(s.openSum) }, suppliers: { receipts: s.supN, sum_aoa: fmt(s.supSum), open_without_customer: s.openSup, open_sum_aoa: fmt(s.openSupSum) }, matched_pairs: Math.floor(s.matched / 2), needs_review: s.review,
