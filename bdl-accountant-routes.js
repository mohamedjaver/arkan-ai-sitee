...
module.exports = function (app, ctx) {
  ...
  async function writeReport() {
    ...
    /* مصدر الربح الفعلي: كونسول التسوية (العمليات المغلقة بالأوقية). صفقات المطابقة تكميلية فقط ولا تُعدّ نقصًا إن كانت بلا سعر */
    const brief = { date: new Date().toISOString().slice(0, 10), profits: deals ? (deals.ops ? { source: 'console_settlement', unit: 'MRU', today: Math.round(deals.ops.today), month: Math.round(deals.ops.month), all_time: Math.round(deals.ops.all), closed_ops: deals.ops.n, other_currencies: deals.ops.other, note: 'هذا هو الربح الفعلي المسجّل في كونسول التسوية (تبويب الأرباح)' } : { source: 'pair_deals', unit: deals.unit, today: deals.today, month: deals.month, deals: deals.n, unpriced_deals: deals.unpriced, avg_margin_pct: deals.avgMargin }) : null, escalation: esc ? esc.levels : null, receipts_total: s.total, customers: { receipts: s.custN, sum_aoa: fmt(s.custSum), open_without_supplier: s.open - s.openSup, open_sum_aoa: fmt(s.openSum) }, suppliers: { receipts: s.supN, sum_aoa: fmt(s.supSum), open_without_customer: s.openSup, open_sum_aoa: fmt(s.openSupSum) }, matched_pairs: Math.floor(s.matched / 2), needs_review: s.review,
    ...
    const rep = await skill.ask(brief, { extra: 'اكتب تقرير اليوم: الوضع، المؤشرات (منها ربح اليوم والشهر من profits — وهو الربح الفعلي من كونسول التسوية إن كان source=console_settlement؛ لا تذكر «غير مسعّر» ولا تعتبر صفقات المطابقة بلا سعر مشكلة)، الذمم (زبائن بلا مقابل حسب الجهة مع مستوى التصعيد 1=إشعار 2=تذكير 3=خطر 4=تقرير إداري)، الإجراءات، المخاطر، ورسالة واتساب لأكبر جهة زبون بلا مقابل — وإن لم توجد إيصالات زبائن فاجعل whatsapp فارغًا وأضف إجراءً: رفع إيصالات الزبائن ثم مطابقة Claude.' });
  ...
  async function dealsSummary() {
    ...
    /* نفس مصدر تبويب «الأرباح» في الكونسول: bdl_profit_daily (المعاملات المنجزة) — بالأوقية، ومعه أي ربح بعملة أخرى منفصلًا */
    try { const ops = await sb('/bdl_profit_daily?select=day,tx_count,profit,profit_ccy&order=day.desc&limit=600'); S.ops = { today: 0, month: 0, all: 0, n: 0, other: {} };
      for (const o of ops) { const d = String(o.day).slice(0, 10); const v = Number(o.profit) || 0; const c = o.profit_ccy || 'MRU';
        if (c === 'MRU') { S.ops.all += v; S.ops.n += Number(o.tx_count) || 0; if (d === t) S.ops.today += v; if (d.slice(0, 7) === m) S.ops.month += v; }
        else { S.ops.other[c] = S.ops.other[c] || { month: 0, all: 0 }; S.ops.other[c].all += v; if (d.slice(0, 7) === m) S.ops.other[c].month += v; } }
      S.ops.today = Math.round(S.ops.today * 100) / 100; S.ops.month = Math.round(S.ops.month * 100) / 100; S.ops.all = Math.round(S.ops.all * 100) / 100; } catch (e) { S.ops = null; }
    return S;
  }
