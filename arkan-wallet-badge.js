...
if(!addr||!/^T[1-9A-HJ-NP-Za-km-z]{33}$/.test(addr)) return;
...
function ensureChip(){
  ...
  if(!chip){
    ...
    chip.style.cssText='position:fixed;top:calc(env(safe-area-inset-top,0px) + 10px);inset-inline-start:12px;z-index:60;'+
  ...
  chip.innerHTML='<span class="wdot" style="width:8px;height:8px;border-radius:50%;background:#5CFFB1;'+
    ...
    '<span style="opacity:.75">USDT</span>'+
...
async function loadBal(){
  ...
  try{
    ...
    var j=await r.json(); var acc=j.data&&j.data[0]; var u=0;
