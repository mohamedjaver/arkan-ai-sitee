...
(function () {
  ...
  .ark-app{display:flex;align-items:center;gap:13px;padding:15px;border-radius:16px;text-decoration:none;
    background:var(--ark-glass,rgba(17,23,40,.72));border:1px solid var(--ark-stroke,rgba(255,255,255,.09));
    color:var(--ark-txt,#F4F6FB);transition:transform .18s,border-color .18s,box-shadow .18s}
  .ark-app:hover{transform:translateY(-3px);border-color:var(--ark-violet,#6C5CE7);box-shadow:0 24px 50px -26px rgba(0,0,0,.7)}
  .ark-app.on{border-color:var(--ark-bai,#EA4E3D)}
  .ark-app-ic{width:44px;height:44px;flex:none;display:grid;place-items:center;border-radius:12px;color:#fff;
    background:var(--ark-grad,linear-gradient(135deg,#8B7BFF,#6C5CE7 55%,#EA4E3D))}
  ...
  .ark-app-tx i{font-style:normal;font-size:12px;color:var(--ark-txt-2,rgba(138,147,168,.9))}
  /* launcher */
  #ark-fab{position:fixed;inset-inline-start:16px;bottom:calc(env(safe-area-inset-bottom,0px) + 80px);z-index:99990;
    ...
    background:var(--ark-grad,linear-gradient(135deg,#8B7BFF,#6C5CE7 55%,#EA4E3D));
    box-shadow:0 14px 34px -12px rgba(108,92,231,.7);transition:transform .18s}
  ...
  #ark-launcher{position:fixed;inset:0;z-index:99991;display:none;align-items:flex-end;justify-content:center;
    background:rgba(4,6,11,.6);backdrop-filter:blur(6px)}
  #ark-launcher.on{display:flex}
  .ark-sheet{width:100%;max-width:680px;max-height:86vh;overflow-y:auto;background:var(--ark-bg-2,#0E1322);
  ...
  .ark-sheet-h .ky{font-size:11px;font-weight:700;letter-spacing:.14em;color:var(--ark-bai,#EA4E3D);text-transform:uppercase}
