...
(function () {
  ...
  .ark-app{display:flex;align-items:center;gap:13px;padding:15px;border-radius:16px;text-decoration:none;
    background:linear-gradient(180deg,#FFFFFF 0%,#F8FBFF 100%);border:1px solid var(--ark-stroke,#D9E5F2);
    color:#102A43!important;transition:transform .18s,border-color .18s,box-shadow .18s}
  .ark-app:hover{transform:translateY(-3px);border-color:#0058D9;box-shadow:0 18px 40px -20px rgba(0,86,214,.18)}
  .ark-app.on{border-color:#0056D6!important}
  .ark-app-ic{width:44px;height:44px;flex:none;display:grid;place-items:center;border-radius:12px;color:#fff;
    background:linear-gradient(135deg,#0058D9,#0099FF 55%,#00B5FF)}
  ...
  .ark-app-tx i{font-style:normal;font-size:12px;color:#486581!important}
  /* launcher */
  #ark-fab{position:fixed;inset-inline-start:16px;bottom:calc(env(safe-area-inset-bottom,0px) + 80px);z-index:99990;
    ...
    background:var(--ark-grad,linear-gradient(135deg,#0D47B5,#2F8FE8 55%,#4FB2F8));
  ...
  #ark-launcher{position:fixed;inset:0;z-index:99991;display:none;align-items:flex-end;justify-content:center;
    background:rgba(16,42,67,.45);backdrop-filter:blur(6px)}
  #ark-launcher.on{display:flex}
  .ark-sheet{width:100%;max-width:680px;max-height:86vh;overflow-y:auto;background:#FFFFFF;
    border:1px solid #D9E5F2;border-radius:26px 26px 0 0;padding:22px 20px calc(env(safe-area-inset-bottom,0px) + 26px);
    box-shadow:0 -24px 70px -28px rgba(16,42,67,.3);animation:arkUp .28s cubic-bezier(.4,0,.2,1)}
  ...
  .ark-sheet-h b{font-weight:800;font-size:18px;color:#071E4F!important}
  .ark-sheet-h .ky{font-size:11px;font-weight:800;letter-spacing:.14em;color:#0056D6!important;text-transform:uppercase}
  .ark-sheet-h button{margin-inline-start:auto;width:36px;height:36px;border-radius:11px;border:1px solid var(--ark-stroke,rgba(255,255,255,.12));
    background:#F0F6FF;color:#486581!important;cursor:pointer;display:grid;place-items:center}
