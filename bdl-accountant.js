...
module.exports = function (app, ctx) {
  ...
  async function run(reason) {
    ...
    try { const st = { reason, at: new Date().toISOString() }; st.match = await matchLedger(); st.audit = await auditBooks(); st.dues = await dues();
      try { const t = new Date().toISOString().slice(0, 10), m = t.slice(0, 7); const P = await sb('/bdl_profit_daily?select=day,tx_count,profit,profit_ccy&order=day.desc&limit=600'); const pr = { source: 'console_settlement', unit: 'MRU', today: 0, month: 0, all: 0, tx: 0, other: {} }; for (const o of P) { const d = String(o.day).slice(0, 10), v = Number(o.profit) || 0, c = o.profit_ccy || 'MRU'; if (c === 'MRU') { pr.all += v; pr.tx += Number(o.tx_count) || 0; if (d === t) pr.today += v; if (d.slice(0, 7) === m) pr.month += v; } else { pr.other[c] = (pr.other[c] || 0) + (d.slice(0, 7) === m ? v : 0); } } pr.today = Math.round(pr.today); pr.month = Math.round(pr.month); pr.all = Math.round(pr.all); st.profits = pr; } catch (e) { st.profits = null; } const text = await writeReport(st);
      STATE.last = st.at; STATE.lastReport = text; STATE.lastError = null;
      /* تصعيد الذمم: يوم 3+ تذكير جاهز بضغطة (رابط واتساب)، يوم 7+ خطر، يوم 14+ إداري */
      try { if (reason === 'daily' || reason === 'manual') { const now = Date.now(); const L = (st.dues.parties || []).map(p => { const d = p.oldest || 0; return Object.assign({}, p, { days: d }); }).filter(p => p.days >= 3).sort((a, b) => b.days - a.days).slice(0, 12);
        if (L.length) { const lines = L.map(p => { const ph = String((p.who.match(/\+?\d[\d\s]{7,}/) || [''])[0]).replace(/\D/g, ''); const lv = p.days >= 14 ? '🔴 إداري' : p.days >= 7 ? '🟠 خطر' : '🟡 تذكير'; const msg = encodeURIComponent('السلام عليكم، لدينا ' + p.n + ' إيصال بقيمة ' + fmt(p.tot) + ' AOA لم يصل مقابلها منذ ' + p.days + ' يوم. نرجو التأكيد. BDL'); return lv + ' <b>' + p.who.replace(/&/g, '&amp;').replace(/</g, '&lt;') + '</b> — ' + fmt(p.tot) + ' AOA · ' + p.days + ' يوم' + (ph ? '\n<a href="https://wa.me/' + ph + '?text=' + msg + '">إرسال تذكير واتساب</a>' : ''); });
