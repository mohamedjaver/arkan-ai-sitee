...
/* ───────────── API + الموقع ───────────── */
const app = express();
app.use(express.json({ limit: '15mb' }));   /* صور الإيصالات المصغّرة تُرسل base64 */
...
app.use((req, res, next) => {
  ...
  res.setHeader('Access-Control-Allow-Headers', 'Content-Type, Authorization, x-gemini-key, x-file-name, x-side, x-bdl-pin, x-agent-token');
